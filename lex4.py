...
class som_:
    ...
    def __init__(self, x, y, dim, masked=True,
        ...
        with tf.name_scope(self.name):
            ...
            #self.nmapsi_ = tf.constant(self.nmaps, self.itype)
        ...
        ## random_binary_features_ = tf.floor(1.0 + tf.random_uniform([1, 1, self.dim], dtype=self.dtype) - self.lesion_strength_)
                                   
        ## self.lesion_lose_features_ = tf.assign(self.W_,
        ##                                                 tf.multiply(self.W_,
        ##                                                             random_binary_features_),
        ##                                                 name="lose_features")
...
class lex_:
    ...
    def __init__(self, sem_data, eng_data, spa_data, x=None, y=None, naming_only=False,
        ...
        if self.naming_only: ### NAMING ONLY OPS
            ...
            ## self.train_trans_both = self.make_assoc_train_op([self.ENG2SPA, self.SPA2ENG])
            
            ## self.train_trans_eng2spa = self.make_assoc_train_op([self.ENG2SPA])
            ## self.train_trans_spa2eng = self.make_assoc_train_op([self.SPA2ENG])
            
            ## self.train_translation = [self.eng.train_, self.spa.train_, 
            ##                           self.train_trans_both] 

            ## self.train_both_no_trans     = [self.sem.train_, self.eng.train_, self.spa.train_, 
            ##                                 self.make_assoc_train_op([self.SEM2ENG, self.ENG2SEM,
            ...
            self.leak_train_spa_, self.leak_act_spa_ = self.make_leak_train_op(lang='spa') #mf.
    ...
    def make_sparse_leak_act(self, lang='eng'):
        ...
        if lang == 'eng':
            ...
            wy_ = self.prop_wy_[self.SPA2ENG] ## arrrrgh
        elif lang == 'spa':
            som = self.spa
            act_ = self.prop_act_[self.ENG2SPA]
            wx_ = self.prop_wx_[self.ENG2SPA]
            wy_ = self.prop_wy_[self.ENG2SPA] ## arrrrgh
    ...
    def make_leak_train_op2(self, lang='eng'):
        ...
        ## really should concatenate 12 and 21 indices and duplicate deltas of course... meh  5/29/18, Uli
        leak_train_ = tf.scatter_nd_add(self.assoc_, indices12_, deltas_, use_locking=False)   
...
class Lex(lex_):
    ...
    def __init__(self, sem_data, eng_data, spa_data, x=None, y=None,
        ...
        #self.semantic_lesion_mask = [[0.0]]
        #self.eng_lesion_mask = [[0.0]]
        #self.spa_lesion_mask = [[0.0]]
    ...
    def load_npz(cls, fname, P, T, W, naming_only=False):
        ...
        npz = np.load(fname)  #"../new_model/test_ep00600.npz")
        params = {key:npz[key] for key in npz.keys()}    
        x, y, _ = params['sem'].shape

        print "SHAPE", x,y

        #naming_only = (params["assoc"].shape[0] == 2)
        ...
        ## if "treatment_seed" in npz:
        ##     T.treatment_seed = int(npz["treatment_seed"])
        ##     cls.log("INFO:      npz treatment_seed: %d"%(T.treatment_seed))   ### we never save treated nets, right?
        ...
        if "lesioned" in npz and bool(npz["lesioned"]):
            ...
            #cls.log("INFO:   assoc:    %r, %r"%(model.assoc_lesion_type, model.assoc_lesion_strength))
    ...
    def init_model(self, P, H, sess=None):
        ...
        ### TODO: this is crazy... we're assuming lesion masks stay the same whenever we reset a lesioned net... well that might be OK? Hmmm
    ...
    def score_naming_alt(self, res, P, W, sess=None, subset=None, return_results=False, test_trans=False):
        ...
        #sess.run(tf.gather_nd(self.eng.labels_, tf.stack([res.enx.values, res.eny.values], axis=-1)), feed)    
    ...
    def score_papt(self, W, thresh=0.0, nbatch=32, sess=None):
        ...
        #res["correct"] = (res.f1-res.fcue).abs()<(res.f2-res.fcue).abs()
    ...
    def test_all(self, P, W, H, T, sess=None, v=True, test_trans=False, **args):
        ...
        #prefix = prefix if prefix is not None else [0] * len(prefix_index)
        ...
        #res.set_index(prefix_index+["word"], inplace=True)
    ...
    def train(self, target_age, P, H, W, sess=None, alpha_factor=1.0):
        ...
        while(self.age<target_age):
            ...
            ## self.log("INFO: minR = %r"%P.minR)
            ## self.log("INFO: maxR = %r"%P.maxR)
            ## self.log("INFO: P.hood_std_devs = %r"%P.hood_std_devs)
            ## self.log("INFO: (Something's None, no?)")
    ...
    def treat_one_session(self, P, H, W, T, sess=None):
        ...
        ##### TODO   TRAIN TO SIMULATE NORMAL EXPOSURE
        ...
        # then train in treatment language
        # and train other language with leak... 

        ### TODO: 
        ## leak_train_map_method = None
        ## leak_train_map_factor = None
        ## leak_train_map_nmax = None
        ## leak_train_map_nmax_sigma = None 


        


        ### note treatment language is part of patient data, not a treatment alternative... this may be future trouble; whatever

        ### first train semantic map, naming in treatment language, and assoc leaks

        ###treatment_normal_exp_weeks
        ###treatment_normal_exp_train_all
        ###leak_use_fake_act
        ...
        treat_feed =  {self.i_: batch,
                       ...
                       #self.map_leak_alpha_: map_alphaf(self.age) * T.
                       self.R_: max(min(T.treatment_hood_size*sigmaf(self.age), P.maxR), P.minR),
                       self.sem.mask_: self.semantic_lesion_mask,
                       self.eng.mask_: self.eng_lesion_mask,
                       self.spa.mask_: self.spa_lesion_mask,
        ...
        if T.noleak and H.treatment_language == 'eng':   ### training english, NOT leaking
            ...
            sess.run(self.train_spanish, treat_feed)
             ...
             ### TODO: run only what's necessary
